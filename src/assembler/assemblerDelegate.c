...
/// Parses the given [line] and add its IR form to [state].
/// @param line The assembly instruction to parse.
/// @param state The assembler state to modify.
...
/// Translates the given [irObject] and returns its binary form..
/// @param irObject The assembly instruction to translate to binary.
/// @param state The assembler state to modify.
Instruction translate(IR *irObject, AssemblerState *state) {
    Instruction instruction = getTranslator(&irObject->type)(irObject, state);
    state->address += 0x4;
    return instruction;
}
